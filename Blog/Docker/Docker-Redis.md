...
详细配置看 redis配置说明
```
mkdir -p /home/devops/data/redis/conf
touch /home/devops/data/redis/conf/redis.conf
```

### 设置redis持久化aof---选做
```
echo "appendonly yes" >> /home/devops/data/redis/conf/redis.conf
```

### 创建Docker网络（如果没有）
```
sudo docker network create -d bridge common-network
```

###  docker启动redis
```
sudo docker run -p 6379:6379 --name redis -v /home/devops/data/redis/data:/data -v /home/devops/data/redis/conf/redis.conf:/etc/redis/redis.conf --restart=always --network common-network -d redis redis-server /etc/redis/redis.conf
```
